...
describe("@creative/shared contracts", () => {
  it("shares the health response schema for server and web", () => {
    const parsed = healthResponseSchema.parse({
      ok: true,
      service: "creative-server",
      version: "0.1.0",
    });

    expect(parsed.ok).toBe(true);
    expect(parsed.service).toBe("creative-server");
  });
  ...
  it("shares the viewer bootstrap contract for GET /api/viewer", () => {
    ...
    const parsed = viewerResponseSchema.parse({
      profile: {
        id: "user_123",
        email: "maker@creative.test",
        displayName: "Creative Maker",
