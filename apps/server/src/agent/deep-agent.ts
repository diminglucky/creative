...
import { CREATIVE_SYSTEM_PROMPT } from "./prompts/creative-main.js";
...
export type CreativeAgent = Pick<
  ReturnType<typeof createDeepAgent>,
  "stream" | "streamEvents"
>;

export type CreativeAgentFactory = (options: {
...
export function createCreativeDeepAgent(options: {
  backendResult?: AgentBackendResult;
  brandKitId?: string | null;
  canvasId?: string;
  checkpointer?: BaseCheckpointSaver;
  ...
  const createUserClient =
    ...
    ((_accessToken: string): never => {
      throw new Error(
        "inspect_canvas is unavailable: no createUserClient was provided to createCreativeDeepAgent.",
      );
    });
