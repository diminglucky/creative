...
export function registerAdminRoutes(app: FastifyInstance, options: { auth: RequestAuthenticator; adminEmail?: string; service: AdminService }) {
  ...
  app.post("/api/admin/credit-packs",async(req,reply)=>{const actor=await guard(req,reply);if(!actor)return;const parsed=creditPackSchema.safeParse(req.body);if(!parsed.success)return reply.code(400).send({error:{code:"invalid_request",message:"充值档位配置无效"}});try{await options.service.createCreditPack(actor,parsed.data);}catch(error){if(isUniqueViolation(error))return reply.code(409).send({error:{code:"invalid_request",message:"充值档位标识已存在"}});throw error;}return reply.code(201).send();});
