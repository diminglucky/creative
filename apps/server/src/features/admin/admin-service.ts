...
export function createAdminService(options: { getAdminClient: () => AdminSupabaseClient; secretCrypto?: ProviderSecretCrypto }) {
  ...
  return {
    ...
    async adjustUser(actor: Actor, id: string, input: any) { const { error } = await client().rpc("adjust_wallet_balance", { p_user_id: id, p_payment_method: "credits", p_amount: input.amount, p_reason: input.reason, p_actor_user_id: actor.id, p_actor_email: actor.email }); if(error) throw error; },
