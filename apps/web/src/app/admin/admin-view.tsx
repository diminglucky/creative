...
export function AdminView<T>({title,load,children}:{title:string;load:(token:string)=>Promise<T>;children:(data:T,reload:()=>void)=>ReactNode}) {
  ...
  return <section><div className="mb-6"><h1 className="text-xl font-semibold">{title}</h1></div>{children(data,()=>setReloadKey((value)=>value+1))}</section>;
}
