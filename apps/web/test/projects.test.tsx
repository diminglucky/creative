...
describe("Projects page", () => {
  ...
  it("renders the new project action with no projects", async () => {
  ...
  it("creates a project from the new project card", async () => {
        ...
        return Promise.resolve({
          ...
          json: async () => ({
            project: {
              ...
              name: "Untitled",
              primaryCanvas: { id: "canvas-new", name: "Main Canvas", isPrimary: true },
            },
          }),
        });
      }
  ...
  it("shows a retry state on a viewer 500", async () => {
    mockFetch.mockImplementation((url: string) => {
      if (url.includes("/api/viewer")) {
        return Promise.resolve({
          ok: false,
