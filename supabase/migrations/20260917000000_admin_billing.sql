...
create policy "workspace members read own wallet" on public.money_wallets for select using (public.is_workspace_member(workspace_id));
create policy "workspace members read own preferences" on public.payment_preferences for select using (public.is_workspace_member(workspace_id));
create policy "workspace members update own preferences" on public.payment_preferences for all using (public.is_workspace_member(workspace_id)) with check (public.is_workspace_member(workspace_id));
create policy "workspace members read own charges" on public.billing_charges for select using (public.is_workspace_member(workspace_id));
create policy "workspace members read own wallet ledger" on public.wallet_ledger for select using (public.is_workspace_member(workspace_id));
create policy "workspace members read own orders" on public.payment_orders for select using (public.is_workspace_member(workspace_id));
...
begin
  ...
  if v_method='credits' then
    ...
    update public.credit_balances set balance=balance-v_price.credit_price, version=version+1, updated_at=now() where workspace_id=p_workspace_id;
    ...
    update public.money_wallets set balance_fen=balance_fen-v_price.money_price_fen, version=version+1, updated_at=now() where workspace_id=p_workspace_id;
...
declare v_charge public.billing_charges%rowtype; v_existing uuid;
begin
  select id into v_existing from public.wallet_ledger where idempotency_key=p_idempotency_key;
  ...
  if v_charge.payment_method='credits' then
    update public.credit_balances set balance=balance+v_charge.credits_charged,version=version+1,updated_at=now() where workspace_id=v_charge.workspace_id;
  else
    update public.money_wallets set balance_fen=balance_fen+v_charge.money_charged_fen,version=version+1,updated_at=now() where workspace_id=v_charge.workspace_id;
  end if;
  ...
  values(v_charge.workspace_id,v_charge.id,'generation_refund',v_charge.payment_method,
    case when v_charge.payment_method='credits' then v_charge.credits_charged else v_charge.money_charged_fen end,0,p_idempotency_key,p_reason)
