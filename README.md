<p align="center">
  <img src="apps/web/public/logo.svg" alt="Creative Logo" width="80" />
</p>
...
<p align="center">
  <img width="900" src="docs/images/base-image.png" alt="Creative" />
</p>
...
<p align="center">
  <img width="900" src="docs/images/home-image.png" alt="Creative Workspace" />
</p>
...
Creative 做的是同一件事，但完全开源。你在无限画布上跟 AI 对话，它直接生成图片、视频，摆好位置，调好样式。不需要时间轴，不需要模板，不需要学 prompt 工程。说"把左边那张换成暖色调"，AI 就懂了。
...
<p align="center">
  <img width="900" src="docs/images/canvas-image.png" alt="Creative Canvas" />
</p>
...
```bash
git clone https://github.com/fancyboi999/Creative.git
cd Creative
pnpm install
```
...
CREATIVE_AGENT_MODEL=google:gemini-2.5-flash     # or openai:gpt-4o
...
|-------|----------|------|---------|
| `free@test.creative.com` | `opensourcecreative` | Free | 50 |
| `starter@test.creative.com` | `opensourcecreative` | Starter | 1,200 |
| `pro@test.creative.com` | `opensourcecreative` | Pro | 5,000 |
| `ultra@test.creative.com` | `opensourcecreative` | Ultra | 15,000 |
...
# Build Command:   pnpm --filter @creative/shared build && pnpm --filter @creative/web build
...
pnpm --filter @creative/server dev:workers:2   # 2 workers (6 concurrent jobs)
pnpm --filter @creative/server dev:workers:3   # 3 workers (9 concurrent jobs)
```
...
| Variable | Description |
|----------|-------------|
| `CREATIVE_AGENT_MODEL` | Agent LLM model (e.g., `google:gemini-2.5-flash`) |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `CREATIVE_SERVER_PORT` | `3001` | API server port |
| `CREATIVE_WEB_ORIGIN` | `http://localhost:3000` | Frontend origin (CORS) |
| `CREATIVE_AGENT_BACKEND_MODE` | `state` | Agent persistence (`state` or `filesystem`) |
| `CREATIVE_SKILLS_ROOT` | `../../skills` | Path to skills directory |
